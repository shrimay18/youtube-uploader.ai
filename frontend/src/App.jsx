...
export default function App() {
  ...
  // After sign-in, prompt for keys if the two required ones aren't set.
  async function checkKeys() {
    try {
      const m = await api.getKeys()
      setNeedsKeys(!m.GEMINI_API_KEY || !m.YOUTUBE_API_KEY)
  ...
  async function resetAccount() {
    if (!confirm('Reset this account on this device? Your locally-encrypted keys are removed and you set up again. (Any keys in a local .env are re-detected.)')) return
    setMenu(false)
    await sbSignOut().catch(() => {})
  ...
  return (
    <div className="app">
              ...
              {menu && (
                <>
                  ...
                  <div className="acct-menu">
                    ...
                    <button onClick={() => { setYtAccounts(true); setMenu(false) }}>▶️ YouTube accounts</button>
                    <button onClick={() => { setSettings(true); setMenu(false) }}>🔑 API keys</button>
                    <button onClick={logout}>↩ Log out</button>
                    <div className="acct-sep" />
                    <button className="danger" onClick={resetAccount}>⚠ Reset account (switch sign-in)</button>
                  </div>
                </>
              )}
            </div>
      ...
      {!status ? (
        ...
        <Landing accountExists={status.account_exists} onGetStarted={goStart} onSignIn={() => setView('login')} />
      )}
