...
export default function Auth({ mode, method, hasGoogle, email, onDone, onBack }) {
  ...
  async function submitPassword() {
    ...
    setBusy('password')
    try {
      // Keys are collected once, after sign-in (KeysOnboarding) — not here.
      if (isSignup) await api.setup(password, {}); else await api.login(password)
      onDone()
    ...
    <div className="split">
      <aside className="split-brand">
        <div className="split-brand-inner">
          ...
          <p>Set up once. Then drop a video and get a ready-to-publish, SEO-optimized post, in your voice and on your schedule.</p>
      ...
      <section className="split-form">
        ...
        <p className="muted" style={{ marginTop: 14, textAlign: 'center' }}>
          🔒 Your password/Google account encrypts your keys locally. No recovery, so keep it safe.
        </p>
      </section>
    </div>
  )
